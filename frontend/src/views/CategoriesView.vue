...
const handleFetchData = async (page = currentPage.value
  ...
  loading.value = true
  await axios.get(`${import.meta.env.VITE_API_URL}/categories?page=${page}`, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(res => {
...
watch(() => currentPage.value, async (to) => {
...
const handleDelete = async () => {
  ...
  await axios.delete(`${import.meta.env.VITE_API_URL}/categories/${dataToDelete.value.id}`, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(async res => {
...
const handleEdit = async () => {
  ...
  await axios.put(`${import.meta.env.VITE_API_URL}/categories/${dataToEdit.value}`, formEdit.value, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(async res => {
...
const formCreate = ref<object>({name: ''})
...
const handleCreate = async () => {
  ...
  await axios.post(`${import.meta.env.VITE_API_URL}/categories`, formCreate.value, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(async res => {
...
<!--          </v-text-field>-->
      ...
      <div class="tw-p-5">
        ...
        <div class="tw-flex tw-flex-col tw-items-center">
          <v-pagination v-model="currentPage" :length="tableData.last_page">
          </v-pagination>
          <p class="tw-text-sm">Mostrando do {{ tableData.from }} ao {{ tableData.to }} de .Total de Registros
            {{ tableData.total }}</p>
          <pre v-if="false">
            {{ tableData }}
          </pre>
    ...
    >
      ...
      >
        <p class="tw-px-6 tw-py-6">
          Você deseje mesmo apagar a categoria <strong v-if="dataToDelete !== null">{{ dataToDelete.name }}</strong>?
    ...
    >
      ...
      >
        <v-form @submit.prevent="handleEdit" class="tw-px-4 tw-py-3">
          <v-text-field
              v-model="formEdit.name"
              hide-details="auto"
              label="Nome"
              :rules="[() => !!formEdit.name || 'O nome é obrigatório']"
              :readonly="loadingEdit" :loading="loadingEdit"
          ></v-text-field>
        </v-form>
    ...
    >
      ...
      >
        <v-form @submit.prevent="handleCreate" class="tw-px-4 tw-py-3">
          <v-text-field
              v-model="formCreate.name"
              hide-details="auto"
              label="Nome"
              :rules="[() => !!formCreate.name || 'O nome é obrigatório']"
              :readonly="loadingCreate" :loading="loadingCreate"
          ></v-text-field>
        </v-form>

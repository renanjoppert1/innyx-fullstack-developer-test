...
const handleFetchData = async (page = currentPage.value
  ...
  loading.value = true
  await axios.get(`${import.meta.env.VITE_API_URL}/users?page=${page}`, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(res => {
...
const handleDelete = async () => {
  ...
  await axios.delete(`${import.meta.env.VITE_API_URL}/users/${dataToDelete.value.id}`, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(async res => {
...
const handleEdit = async () => {
  ...
  await axios.put(`${import.meta.env.VITE_API_URL}/users/${dataToEdit.value}`, formEdit.value, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(async res => {
...
const handleCreate = async () => {
  ...
  await axios.post(`${import.meta.env.VITE_API_URL}/users`, formCreate.value, {headers: {authorization: `Bearer ${localStorage.getItem('token')}`}}).then(async res => {
    ...
    >
      ...
      >
        <p class="tw-px-6 tw-py-6">
          Você deseje mesmo apagar o usuário <strong v-if="dataToDelete !== null">{{ dataToDelete.name }}</strong>?
